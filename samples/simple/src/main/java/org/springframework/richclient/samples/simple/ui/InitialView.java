...
/**
 * This class defines the initial view to be presented in the sample application. It is
 * constructed automatically by the platform and configured according to the bean
 * specification in the application context. Here's an example configuration:
 * 
 * <pre>
 *    &lt;bean id=&quot;initialView&quot;
 ...
 * <code>viewProperties</code> and it takes a map of key/value pairs. Each key is the
 * name of a property to be set on the actual view class (this class) and the value is the
 * value to set for that property. So, two properties have been configured,
 * <code>firstMessage</code> and <code>descriptionTextPath</code>. The
 * <code>firstMessage</code> value specifies the key of a message to be displayed and
 * the <code>descriptionTextPath</code> specifies the path to a file containing the text
...
public class InitialView extends AbstractView {
    ...
    /**
     * Set the resource that references the file containing the description text to place
     * in the description areas of this view. Note that even though this property is of
     * type Resource, the Spring platform will automatically convert a string path into a
     * resource.
     * 
