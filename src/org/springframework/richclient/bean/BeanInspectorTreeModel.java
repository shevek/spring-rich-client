...
public class BeanInspectorTreeModel extends AbstractTreeTableModel {
	...
	public static class BeanFeatureNode {
		...
		static {
			TYPE_THEN_NAME_COMPARATOR.addComparator(new PropertyComparator("simpleScalar", BooleanComparator.instance()));
			TYPE_THEN_NAME_COMPARATOR.addComparator(new PropertyComparator("name", String.CASE_INSENSITIVE_ORDER));
		}
		...
		public BeanFeatureNode(Object bean) {
			...
			try {
				this.feature = Introspector.getBeanInfo(bean.getClass()).getBeanDescriptor();
			}
			catch (Exception e) {
				logger.error("Unable to introspect bean: " + bean.getClass(), e);
			}
		}
		...
		public Object[] getChildren() {
			...
			if (bean == null || ClassUtils.isSimpleScalar(bean.getClass())) {
				...
				this.children = (BeanFeatureNode[])tmpList.toArray(new BeanFeatureNode[tmpList.size()]);
				return children;
			}
		}
