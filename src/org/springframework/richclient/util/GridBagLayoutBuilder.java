...
/**
 * This provides an easy way to create panels using a {@link GridBagLayout}.<p />
 *
 * Usage is:<pre>
...
public class GridBagLayoutBuilder {
  ...
  /**
   * Returns the {@link ComponentFactory} that this uses to create things
   * like labels.
   *
  ...
  /**
   * Sets the {@link ComponentFactory} that this uses to create things
   * like labels.
   */
  ...
  public GridBagLayoutBuilder append(Component component, int colSpan, int rowSpan, boolean expandX, boolean expandY,
                                     Insets insets) {
    if (expandX && expandY)
      return append(component, 1, 1, 1.0, 1.0, insets);
    else if (expandX)
      return append(component, 1, 1, 1.0, 0.0, insets);
    else if (expandY)
      return append(component, 1, 1, 0.0, 1.0, insets);
    else
      return append(component, 1, 1, 0.0, 0.0, insets);
  }
  ...
  public GridBagLayoutBuilder append(Component component, int colSpan, int rowSpan, double xweight, double yweight,
                                     Insets insets) {
    final GridBagConstraints gbc = createGridBagConstraint(colSpan, rowSpan, xweight, yweight, insets);
  ...
  /**
   * Appends a right-justified label to the end of the given line, using
   * the provided string as the key to look in the
   * {@link #setComponentFactory(ComponentFactory) ComponentFactory's}
   * message bundle for the text to use.
   *
   * @param labelKey the key into the message bundle; if not found the key
   *                 is used as the text to display
   *
   * @return "this" to make it easier to string together append calls
   */
  ...
  /**
   * Appends a left-justified label to the end of the given line, using
   * the provided string as the key to look in the
   * {@link #setComponentFactory(ComponentFactory) ComponentFactory's}
   * message bundle for the text to use.
   *
  ...
  /**
   * Appends the given component to the end of the current line.
   * The component will "grow" horizontally as space allows.
   *
   ...
   * Has an implicit {@link #nextLine()} before and after it.
   *
   * @return "this" to make it easier to string together append calls
   */
  ...
  /**
   * Appends a seperator (usually a horizonal line) using
   * the provided string as the key to look in the
   * {@link #setComponentFactory(ComponentFactory) ComponentFactory's}
   * message bundle for the text to put along with the seperator.
   * Has an implicit {@link #nextLine()} before and after it.
   *
   * @return "this" to make it easier to string together append calls
   */
  ...
  private GridBagConstraints createGridBagConstraint(int colSpan, int rowSpan, double xweight, double yweight,
                                                     Insets insets) {
  ...
  public JPanel getPanel() {
    ...
    final JPanel panel = this.showGuidelines ?
        new GridBagLayoutDebugPanel() :
   ...
   * the end of the panel?<p />
   *
   * For example, if you have<pre>
   * append(a).append(b).append(c).nextLine();
   * append(d).append(e).nextLine();</pre>
