...
public class ListenerListHelper implements Serializable {
	...
	private static final Map methodCache = new CachingMapTemplate() {
		protected Object create(Object o) {
				...
				if (method.getName().equals(key.methodName) && method.getParameterTypes().length == key.numParams) {
					if (fireMethod != null) {
						throw new UnsupportedOperationException("Listener class [" + key.listenerClass
								+ "] has more than 1 implementation of method [" + key.methodName + "] with [" + key.numParams
								+ "] parameters.");
					}
					fireMethod = method;
				}
			...
			if (fireMethod == null) {
				throw new IllegalArgumentException("Listener class [" + key.listenerClass + "] does not implement method ["
	...
	public String toString() {
		return new ToStringCreator(this).append("listenerClass", listenerClass).append("listeners", listeners).toString();
	}
	...
	private static class MethodCacheKey {
		...
		public boolean equals(Object o2) {
			...
			return listenerClass.equals(k2.listenerClass) && methodName.equals(k2.methodName) && numParams == k2.numParams;
		}
