...
public class SwingFormModel extends ApplicationServicesAccessor implements FormModel, PropertyChangePublisher {
	...
	private ValueModel newNestedAspectAdapter(ValueModel parentValueHolder, String childProperty) {
		...
		PropertyAdapter adapter = new PropertyAdapter(strategy.newPropertyAccessStrategy(parentValueHolder), childProperty);
		return adapter;
	}
	...
	public JTextComponent bindAsLabel(final JTextComponent component, String parentProperty, String childPropertyToDisplay) {
	...
	public ObservableList createBoundListModel(String formProperty) {
		...
		if (valueModel == null) {
			...
			valueModel = new BufferedCollectionValueModel(adapter, getMetadataAccessStrategy().getPropertyType(formProperty));
	...
	public JList bind(JList list, String selectionFormProperty, ValueModel selectableItemsHolder,
			Comparator itemsComparator) {
		return (JList)bindControl(bind(list, getOrCreateDisplayValueModel(selectionFormProperty), selectableItemsHolder,
				itemsComparator), selectionFormProperty);
	}
