...
public class UIManagerConfigurer {
    ...
    public UIManagerConfigurer(boolean installPrePackagedDefaults) {
        ...
        try {
            ...
            throw new ApplicationException(
                    "Unable to install subclass custom defaults", e);
        }
    }
    ...
    public void setInstallCustomLookAndFeels(String[] customLookAndFeels) {
        for (int i = 0; i < customLookAndFeels.length; i++) {
            String[] feels = StringUtils
                    .commaDelimitedListToStringArray(customLookAndFeels[i]);
            Assert
                    .isTrue(feels.length > 0,
                            "LookAndFeelInfo definition should be in form: [name],<classname>");
            String name = null;
            String className;
            if (feels.length == 1) {
                className = feels[0];
        ...
        try {
            if (lookAndFeelName.equalsIgnoreCase(SYSTEM_LOOK_AND_FEEL_NAME)) {
                UIManager.setLookAndFeel(UIManager
                    ...
                    .equalsIgnoreCase(CROSS_PLATFORM_LOOK_AND_FEEL_NAME)) {
                UIManager.setLookAndFeel(UIManager
