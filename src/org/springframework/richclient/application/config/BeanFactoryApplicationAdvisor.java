...
public class BeanFactoryApplicationAdvisor extends ApplicationAdvisor implements BeanFactoryAware {
	...
	public void afterPropertiesSet() throws Exception {
		...
		Assert.state(beanFactory.containsBean(startingPageId), "Do not know about bean definition with name '"
	...
	public ApplicationWindowCommandManager createWindowCommandManager() {
		this.currentWindowCommands = new XmlBeanFactory(commandFactoryResource, Application.services().getBeanFactory());
		this.currentWindowCommands.addBeanPostProcessor(new ApplicationWindowSetter(getManagedWindow()));
		this.currentWindowCommands.addBeanPostProcessor(newObjectConfigurer());
		registerBeanPostProcessors();
		return (ApplicationWindowCommandManager)currentWindowCommands.getBean(commandManagerBeanName,
	...
	private void registerBeanPostProcessors() throws BeansException {
		String[] beanNames = getCommandsBeanFactory().getBeanDefinitionNames(BeanPostProcessor.class);
		if (beanNames.length > 0) {
			...
			for (Iterator it = beanProcessors.iterator(); it.hasNext();) {
				getCommandsBeanFactory().addBeanPostProcessor((BeanPostProcessor)it.next());
			}
		}
	}
	...
	public CommandGroup getMenuBarCommandGroup() {
		if (menuBarBeanName == null || !currentWindowCommands.containsBean(menuBarBeanName)) {
	...
	public CommandGroup getToolBarCommandGroup() {
		if (toolBarBeanName == null || !currentWindowCommands.containsBean(toolBarBeanName)) {
