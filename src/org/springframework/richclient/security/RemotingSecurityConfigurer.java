        ...
        for (int i = 0; i < factories.length; i++) {
            if (logger.isDebugEnabled()) {
                logger.debug("Updating " + factories[i].toString()
                    + " to username: " + username + "; password: [PROTECTED]");
            }

            try {
                Method method = factories[i].getClass().getMethod("setUsername",
            ...
            try {
                Method method = factories[i].getClass().getMethod("setPassword",
                        new Class[] {String.class});
