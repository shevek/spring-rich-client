 ...
 * </p>
 * 
 * TODO There's a third option: only negative numbers, this should be configurable as well.
 * 
 * @author jh
 * 
 */
public class BigDecimalTextField extends JTextField {
    ...
    /**
     * @see #BigDecimalTextField(int, int, boolean, Class, NumberFormat, NumberFormat)
     */
    ...
    /**
     * When parsing a number, BigDecimalFormat can return numbers different than BigDecimal.
     * 
     * @param format
     * @param numberClass
     * 
    ...
    {
        if (format instanceof DecimalFormat && (numberClass == BigDecimal.class))
        {
    ...
    class BigDecimalDocument extends PlainDocument
    {
        ...
        /**
         * Handles string insertion, checks several things like number of non-decimals/decimals/sign...
         * 
         * @inheritDoc
         */
                ...
                throws BadLocationException
        {
            ...
            // check decimal signs
            else if((str.length() == 1) && ((this.alternativeSeparator == str.charAt(0)) || (this.decimalSeparator == str.charAt(0))) )
            {
                if ((nrOfDecimals > 0) && (nrOfDecimals >= (getLength() - offset)) && (getText(0, getLength()).indexOf(this.decimalSeparator) == -1))
                {
            ...
            for (int i = 0; i < sarr.length; i++) {
                if (sarr[i] == this.decimalSeparator) 
                {
                    ...
                    {// double decimalseparator??
                        log.warn("Error while inserting string: " + s + "[pos=" + i + "]" + " Double decimalseparator?");
                        return;
                    }
                    sep = i;
                    if (numberLength > this.nrOfNonDecimals)
                    {// too many digits left of decimal separator
                        log.warn("Error while inserting string: " + s + "[pos=" + i + "]" + " Too many non decimals? [" + this.nrOfNonDecimals + "]");
                        return; 
                    }
                    else if ((sarr.length - sep - 1) > this.nrOfDecimals)
                    {// too many digits right of decimal separator
                        log.warn("Error while inserting string: " + s + "[pos=" + i + "]" + " Too many decimals? [" + this.nrOfDecimals + "]");
                        return; 
                    }
                } 
                ...
                {// non digit, no grouping/decimal separator not allowed
                    log.warn("Error while inserting string: " + s + "[pos=" + i + "]" + " String contains character that is no digit or separator?");
                    return;
                }
                else
                    ++numberLength;
