 ...
 */
public class ExtTabbedDialogPage extends CompositeDialogPage {
	private Map page2tab = new HashMap();

	private ExtTabbedPane tabbedPaneView;

	public ExtTabbedDialogPage(String pageId) {
		super(pageId);
	}

	public ExtTabbedDialogPage(String pageId, boolean autoConfigure) {
	...
	protected JComponent createControl() {
		...
		for (int i = 0; i < pages.size(); i++) {
			...
			page.addPropertyChangeListener("visible", new PropertyChangeListener() {
				public void propertyChange(PropertyChangeEvent event) {
					if (event.getPropertyName().equals("visible")) {
						tab.setVisible(((Boolean) event.getNewValue()).booleanValue());
					}
				}
			});
		}
		tabbedPane.setModel(new DefaultSingleSelectionModel() {
			public void setSelectedIndex(int index) {
				...
				if (canChangeTabs()) {
					...
					if (index >= 0) {
						ExtTabbedDialogPage.super.setActivePage((DialogPage) getPages().get(index));
					}
					else {
						ExtTabbedDialogPage.super.setActivePage(null);
					}
				}
			}
		});
